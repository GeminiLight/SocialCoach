...
状态：已完成。专用飞书应用、收件表、ModelScope 与 Vercel 均已上线，真实提交验收通过；2026-09-17 按用户要求为本人账号添加整张 Base 的可管理权限，并读回确认。
...
- 创建时表协作者仅专用应用；2026-09-17 为本人（王天富）添加 `full_access`（可管理）权限，重新读取协作者列表确认应用与本人均有可管理权限。应用仍为所有者。反馈接收已可用，不配置任何群发通知。
