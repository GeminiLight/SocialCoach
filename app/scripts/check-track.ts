...
async function main() {
  ...
  const start = { name: "session_start", ts, session: "abc123def456", scenario: "neighbor-noise", origin: "arena", context: "community", difficulty: 2, timed: true };
  const end = { name: "session_end", ts: ts + 90_000, session: "abc123def456", scenario: "neighbor-noise", outcome: "partial", turns: 5, silences: 1, duration_s: 90, ended_by: "engine" };
  ...
  try {
    ...
    globalThis.fetch = async (url, init) => {
      ...
      if (/\/tables$/.test(u)) {
        creates++;
        const t = { table_id: `tbl${creates}`, name: JSON.parse(String(init?.body)).table.name };
        tables.push(t);
    ...
    assert.equal((await POST(request(batch([{ name: "app_open", ts: ts + 1 }])))).status, 202);
    await settle();
    assert.equal(creates, 1); assert.equal(lists, 1); assert.equal(writes, 2);

    // a new month gets its own table; a flush across the boundary writes to both
    const oct = Date.UTC(2026, 9, 1, 0, 0, 1);
    assert.equal((await POST(request(batch([{ name: "app_open", ts: ts + 2 }, { name: "app_open", ts: oct }])))).status, 202);
    await settle();
    assert.equal(creates, 2); assert.equal(tables[1].name, "events_2026_10"); assert.equal(writes, 4);
    ...
    // a pinned table skips discovery
    ...
    for (let i = 0; i < 260; i++) {
      const r = await POST(request(batch([{ name: "app_open", ts }]), {}, "198.51.100.7"));
      if (r.status === 429) { limited = r; break; }
    }
    assert.ok(limited, "the per-address window closes"); assert.equal(limited!.headers.get("Retry-After"), "3600");
    console.log("Track checks passed: configuration, strict schema, privacy, monthly tables, idempotency, failure tolerance, pinned table, rate limiting.");
  } finally {
