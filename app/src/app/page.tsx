...
import { compColor, hueColor, relDate } from "@/lib/format";
      ...
      <Page className="pt-4 flex flex-col gap-8">
        ...
        <header className="flex items-start justify-between gap-4">
          <div>
            ...
            <h1 className="display text-[30px] leading-tight mt-1">{t(lang, "home_today")}</h1>
          </div>
        ...
        <section className="flex flex-col gap-3">
          ...
          <ul className="card divide-y divide-line">
            {profile.goals.map((g) => {
              const s = skillById(g);
              return (
                <li key={g} className="flex items-center justify-between gap-3 px-4 h-12">
...
function TodayCard({ session, onStart, onSwap }: { session: ReturnType<typeof buildSession>; onStart: () => void; onSwap: () => void }) {
  ...
  return (
    <motion.article initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }} className="card overflow-hidden">
      <div className="relative h-40" style={{ background: hueColor(sc.characters.find((c) => c.id !== "you" && !c.playable)?.hue ?? 40, 0.9, 0.05) }}>
        <ScenarioCover scenario={sc} size={160} full />
      ...
      <div className="p-5 flex flex-col gap-4">
        <div>
          <h2 className="display text-[24px] leading-tight">{sc.title[lang]}</h2>
          <p className="text-[14px] text-ink-2 mt-2 leading-relaxed">{sc.hook[lang]}</p>
        </div>
          ...
          <div>
            ...
            <div className="grid transition-[grid-template-rows] duration-300" style={{ gridTemplateRows: why ? "1fr" : "0fr" }}>
              <div className="overflow-hidden">
                <p className="text-[14px] text-ink-2 leading-relaxed bg-teal-soft rounded-xl px-4 py-3 mt-1">{session.adaptation?.why || session.prescription?.rationale}</p>
              </div>
            </div>
          </div>
        )}
